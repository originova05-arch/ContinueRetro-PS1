...
Reuse v0.6 translation calls. Technical checks, model review, bounded selective
repair and explicit human approval remain separate persisted operations.
"""
...
def selection(s,pid,kind,scope):
    ...
    if kind not in {'quality_review','quality_technical'}:
        sql+=' AND (w.linguistic_revision IS NULL OR w.linguistic_revision<>w.revision) AND (w.approved_revision IS NULL OR w.approved_revision<>w.revision)'
    return sql,args


def preflight(s,pid,kind,scope=None):
    sql,args=selection(s,pid,kind,scope)
    with s.connect() as db:count=db.execute('SELECT COUNT(*) FROM work_items w WHERE '+sql,args).fetchone()[0]
...
def sync_queue_after_legacy(db,jid):
    """Called within the existing translator's commit transaction by a small hook."""
...
def policy_now(s,pid,store):
    return {'preferences':repo.preferences(s,pid),'glossary_version':store.policy_version(s,pid)}
...
def create_run(t,kind,options):
    ...
    if resume:
        ...
        if policy!=policy_now(s,t.pid,store):raise ValueError('ศัพท์/เกณฑ์เปลี่ยนหลังพักคิว เริ่มรอบใหม่โดยไม่ใช้ผลจากกฎเก่า')
    ...
    cfg=dict(store.settings(s));cfg['_digests']={};cfg['_force']=options.get('force',False)
    ...
    if kind!='quality_technical':
        client=Client(cfg['endpoint'],cfg['timeout_seconds']);names={cfg.get('review_model') or cfg['model']}
    ...
    sql,args=selection(s,t.pid,kind,options.get('scope'))
    rid=uuid.uuid4().hex;stamp=repo.now();policy=policy_now(s,t.pid,store)
...
def candidate_review(s,r,client,model,cfg,t):
    """Review a proposed next revision without assigning its score to old text."""
...
def better_candidate(old,new):
    """Conservative advisory comparison; not an independent human verdict."""
    old_issues=old['quality']['issues'];old_major=sum(x['severity'] in {'major','critical'} for x in old_issues)
...
def polish_once(t,r,cfg,client,models,old,run_id):
    s=t.storage;store,pipeline,_,_,_=dependencies();stage='repair' if old['meaning_blocked'] else 'polish'
    ...
    with s.connect() as db:
        ...
        # Reserve before networking; interrupted inference must not create an unbounded loop.
    ...
    t.check()
    with s.connect() as db:
        db.execute('BEGIN IMMEDIATE');repo.ensure_current(db,r);f=repo.facts(s,r,db)
        if (f['language_hash']!=reviewed.identity.language_policy_sha256 or f['context_hash']!=reviewed.identity.context_sha256 or f['rubric'].digest!=reviewed.identity.rubric_sha256):
            raise ValueError('บริบท/ศัพท์/เกณฑ์เปลี่ยน ไม่ใช้ข้อเสนอเก่าทับฉบับปัจจุบัน')
    ...
    repo.event(s,t.pid,t.jid,'polish','committed',reason,r['id']);t.result['polished']+=1
    return True
...
def run(t,kind,options):
    ...
    try:
        ...
        run_info,cfg,policy=create_run(t,kind,options);rid=run_info['id'];t.continuous_queue=True
        ...
        while True:
            t.check()
            if policy!=policy_now(s,t.pid,store):raise reviewer.ReviewServiceError('เกณฑ์หรือศัพท์เปลี่ยน พักคิวเพื่อไม่ใช้กฎคละกัน')
            with s.connect() as db:
            ...
            for r in batch:
                ...
                if kind not in {'quality_review','quality_technical'} and (r['linguistic_revision']==r['revision'] or r['approved_revision']==r['revision']):
                    mark(r,'done','ผู้ใช้ตรวจภาษา/อนุมัติแล้ว ไม่แก้อัตโนมัติ');continue
                ...
                try:
                    if not r['translation']:
                        ...
                        pipeline._call(t,client,cfg,models,'translate',[r],directory);r=s.work_item(r['id'])
                    ...
                    # Explicit Review may score technically invalid text independently.
                    # Auto/Polish instead parks it: linguistic rewriting is not a codec fix.
                    if technical.blocks_language_operation(report) and kind!='quality_review':
                        mark(r,'review','Technical FAIL: แก้ตัวแปร/codec/ขอบเขตก่อน ไม่เกลาภาษาเพื่อฝืนผ่าน');continue
                    current=repo.detail(s,t.pid,r['id'])
                    if not current['quality']['current'] or cfg.get('_force',False):
                        t.result['requests']+=1
                    ...
                    if kind in {'quality_auto','quality_improve'} and score is not None and current['context']['context_sufficient'] and needs_polish and prefs['auto_repair_rounds']:
                        polish_once(t,s.work_item(r['id']),cfg,client,models,current,rid);current=repo.detail(s,t.pid,r['id'])
                    elif not needs_polish and current['quality']['current'] and score is not None:t.result['skipped_good']+=1
                ...
                progress()
            # Even skipped/technical-only rows advance the visible progress.
            progress()
        ...
        pause();t.result.update(terminal_status='paused',message=str(exc),complete=False,warnings=[str(exc)])
